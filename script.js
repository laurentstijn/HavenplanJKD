...
function saveBoot() {
  ...
  let nieuweLigplaats = document.getElementById('popupLigplaats').value;
...
function editBoot(id) {
  database.ref('boten/' + id).once('value').then(snapshot => {
    ...
    document.getElementById('popupLigplaats').value = boot.ligplaats || "";
    document.getElementById('popup').style.display = 'block';
  });
}
...
function bevestigBoot() {
  ...
  const ligplaats = document.getElementById('popupLigplaats').value;
